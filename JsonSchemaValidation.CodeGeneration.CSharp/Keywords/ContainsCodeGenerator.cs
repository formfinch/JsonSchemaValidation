...
/// </summary>
public sealed class ContainsCodeGenerator : ICSharpKeywordCodeGenerator
{
    ...
    public string GenerateCode(CSharpCodeGenerationContext context)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine($"        if ({context.GenerateValidateCallForItem(containsHash, "_containsItem_", "_containsIdx_")})");
        sb.AppendLine("        {");
        sb.AppendLine("            _containsCount_++;");
        if (trackAnnotations)
        {
            sb.AppendLine($"            {eval}.MarkItemEvaluated({loc}, _containsIdx_);");
        }
