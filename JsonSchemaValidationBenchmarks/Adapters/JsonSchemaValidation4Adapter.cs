...
/// <summary>
/// Adapter that forces Draft 4 for all schemas, enabling fair comparison with other draft versions.
/// </summary>
public sealed class JsonSchemaValidation4Adapter : IPreparsedSchemaValidatorAdapter
{
    // Use the same URI format as the internal system (without trailing #)
    private const string DraftUri = "http://json-schema.org/draft-04/schema";

    public string Name => "JSV-Draft4";
    ...
    private static void ReplaceDraftReferences(JsonNode node)
    {
        if (node is JsonObject obj)
        {
            ...
            if (obj.TryGetPropertyValue("$ref", out var refNode) && refNode is JsonValue refValue)
            {
                ...
                // Replace full meta-schema URIs to Draft 4 format
            ...
            // Replace $id with id for Draft 4 compatibility
            ...
            // Replace $defs with definitions for Draft 4 compatibility
            ...
            // Convert prefixItems + items to Draft 4's items array + additionalItems
            // Draft 2020-12: prefixItems for positional, items for additional
            // Draft 4: items as array for positional, additionalItems for additional
            ...
            // Draft 4 uses a single dependencies keyword
            ...
            // Throw for keywords that cannot be converted to Draft 4
            // This ensures the benchmark test is skipped rather than giving Draft 4 an unfair advantage
            if (obj.ContainsKey("$dynamicRef"))
                throw new NotSupportedException("$dynamicRef is not supported in Draft 4");
            if (obj.ContainsKey("$dynamicAnchor"))
                throw new NotSupportedException("$dynamicAnchor is not supported in Draft 4");
            if (obj.ContainsKey("$recursiveRef"))
                throw new NotSupportedException("$recursiveRef is not supported in Draft 4");
            if (obj.ContainsKey("$recursiveAnchor"))
                throw new NotSupportedException("$recursiveAnchor is not supported in Draft 4");
            if (obj.ContainsKey("minContains"))
                throw new NotSupportedException("minContains is not supported in Draft 4");
            if (obj.ContainsKey("maxContains"))
                throw new NotSupportedException("maxContains is not supported in Draft 4");
            if (obj.ContainsKey("unevaluatedItems"))
                throw new NotSupportedException("unevaluatedItems is not supported in Draft 4");
            if (obj.ContainsKey("unevaluatedProperties"))
                throw new NotSupportedException("unevaluatedProperties is not supported in Draft 4");
            if (obj.ContainsKey("if"))
                throw new NotSupportedException("if/then/else is not supported in Draft 4");
            if (obj.ContainsKey("contentEncoding"))
                throw new NotSupportedException("contentEncoding is not supported in Draft 4");
            if (obj.ContainsKey("contentMediaType"))
                throw new NotSupportedException("contentMediaType is not supported in Draft 4");
            // Draft 6+ only keywords
            if (obj.ContainsKey("const"))
                throw new NotSupportedException("const is not supported in Draft 4");
            if (obj.ContainsKey("contains"))
                throw new NotSupportedException("contains is not supported in Draft 4");
            if (obj.ContainsKey("propertyNames"))
                throw new NotSupportedException("propertyNames is not supported in Draft 4");
